...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            TriggerEvent('mhrp_charCreation:outfits', 1, tonumber(args[1]), args[2])
        else
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('mhrp_charCreation:outfits', 3, tonumber(args[1]))
        ...
        TriggerServerEvent("mhrp_charCreation:list_outfits")
        TriggerEvent("notification", "Not Near A Clothing Store", 2)
    end
end, false) 
...
RegisterCommand("removeoutfit", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('mhrp_charCreation:outfits', 2, tonumber(args[1]))
        else
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("mhrp_charCreation:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("mhrp_charCreation:insert_character_face", data)
            TriggerServerEvent("mhrp_charCreation:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        if nearcloth < 5.0 and jailcheck == 512001 then
            StoreCost = 0;
            menu = {"clothesmenu", "Press ~g~G~s~ to change in to prison outfits"}
        elseif nearcloth < 5.0 then
            menu = {"clothesmenu", "Press ~g~G~s~ to change Clothes $"..StoreCost}
        elseif neartat < 5.0 then
            menu = {"tattoomenu", "Press ~g~G~s~ to change Tattoos $"..StoreCost}
        elseif nearbarber < 5.0 then
            menu = {"barbermenu", "Press ~g~G~s~ to visit the Barber $"..StoreCost}
        ...
        if (menu ~= nil) then
            ...
            if menu[1] == "healmenu" then
                if IsControlJustPressed(1, 188) then -- ARROW UP / DPAD UP
            ...
            if (not enabled) then
                ...
                if (IsControlJustReleased(1, 25)) then -- RIGHT MOUSE BUTTON
...
RegisterNetEvent("mhrp_charCreation:inService")
AddEventHandler("mhrp_charCreation:inService", function(service)
    isService = service
end)


RegisterNetEvent("mhrp_charCreation:hasEnough") -- For Stores
AddEventHandler("mhrp_charCreation:hasEnough", function(menu)
    if menu == "tattoomenu" then
        TriggerServerEvent("mhrp_charCreation:retrieve_tats")
...
RegisterNetEvent("mhrp_charCreation:setclothes")
AddEventHandler("mhrp_charCreation:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            ...
            --DoScreenFadeIn(50)
            --[[ NotUsing Apartment System this can be removed but lets keep it for inspiration #MCS
    ...
    SetClothing(data.drawables, data.props, data.drawtextures, data.proptextures)
    Citizen.Wait(500)
    TriggerServerEvent("mhrp_charCreation:get_character_face")
    TriggerServerEvent("mhrp_charCreation:retrieve_tats")
end)

RegisterNetEvent("mhrp_charCreation:AdminSetModel")
AddEventHandler("mhrp_charCreation:AdminSetModel", function(model)
...
RegisterNetEvent("mhrp_charCreation:settattoos")
AddEventHandler("mhrp_charCreation:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("mhrp_charCreation:setpedfeatures")
AddEventHandler("mhrp_charCreation:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterNetEvent('mhrp_charCreation:outfits')
AddEventHandler('mhrp_charCreation:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("mhrp_charCreation:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("mhrp_charCreation:remove_outfit",pId)
    elseif pAction == 3 then 
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("mhrp_charCreation:get_outfit", pId)
    else
        TriggerServerEvent("mhrp_charCreation:list_outfits")
    end
end)
